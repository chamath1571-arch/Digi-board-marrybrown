...
function calculateSauceStatus(batch: SauceBatch): SauceBatchStatus {
...
function computeExpiryDate(createdAt: string, sauceType: SauceType): string | null {
...
app.post('/orders', (req, res) => {
  ...
  const order: Order = {
    ...
    createdBy,
    totals: {
      itemCount: items.reduce((sum: number, item: any) => sum + item.qty, 0)
    }
  };
